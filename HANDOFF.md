...
## 当前最新快照（2026-09-16，优先阅读）

### 本轮任务：独立两周排班脚本对齐本地页面口径

- 仅修改了仓库外脚本：`G:\浏览器下载\autoschedule-real-two-week.mjs`。
- 未修改产品代码、测试、`spec.md`、`README.md`；未 commit、未 push。
- 这里的“浏览器/页面”特指通过 `127.0.0.1` 打开的本地排班系统页面，不是外部网站。
- 脚本支持两种输入模式：
  - `--zero --excel "Excel路径"`：读取 Excel 配置，强制清空历史、当前班表和值班覆盖，只保留 Excel 配置及人工次数修正。
  - `--state "autoschedule-state.json路径"`：按页面的 `loadState` 恢复历史、当前班表、规则、人工修正和当前日期。
- 默认按项目工作班间隔推进（默认每次加 2 天），不是连续自然日；可用 `--interval` 调整，`--runs` 指定工作班数量，`--start` 指定起始日期。
- 排完每天后复用页面相同的归档和班表安装入口：`currentScheduleHistory`、`replaceHistoryForDate`、`installGeneratedSchedule`。
- 统计复用页面月度统计入口，并分别输出“本次新增次数”和“历史 + 当前班表 + 人工修正”的累计口径。
- 支持页面临时半休参数：`--half-rest "人员=late-start"` 或 `--half-rest "人员=early-finish"`，可重复传入；从零运行默认不带半休。
- 每天输出 TR121/H02、KE166 机动督导的最终人员/空缺、`decisionTrace`、warning、岗位提示和候选拒绝诊断。
...
node "G:\浏览器下载\autoschedule-real-two-week.mjs" --zero --excel "G:\浏览器下载\排班工具配置 (24).xlsx" --start 2026-09-21 --runs 14 --interval 2
```

- 运行 14 个工作班：2026-09-21 至 2026-10-17。
- 初始历史 0 条；半休参数为空；Excel 读取到 3 条 2026-08 人工修正，修正合计 12。
- 14 班全部生成成功；部分班表有 1～2 个待补位；单班耗时约 30.06～210.36 秒；脚本未崩溃。
- TR121/H02：11 名合格人员均至少 1 次，只有完成第一轮后出现第 2 次，说明 H02 分类本身均衡。
- 已验证脚本语法：`node --check "G:\浏览器下载\autoschedule-real-two-week.mjs"` 通过；帮助参数正常。

### 尚未验证与边界

- `--state` 模式尚未实际长跑；半休参数尚未实际长跑。
- 脚本调用项目公共排班入口，但不模拟页面 Worker、DOM 渲染或真实点击，因此不能替代本地页面验收。
- KE166 候选诊断中，同航班柜台兼任可能显示 `time-conflict`，需结合 `decisionTrace` 判断；不代表实际排班错误。
- 脚本位于仓库外，Git 不会跟踪它。

### 当前工作区事实

- 分支：`全局化highs`。
- 仓库内当前有未提交修改：`history.md` 新增“独立排班脚本必须复用本地排班页面口径”教训。
- `.tmp/` 是已有未跟踪诊断目录，不要误删或纳入本轮交接。
- 本轮没有任何产品代码改动。

### 新窗口建议起点

1. 先读取 `AGENTS.md`、本文件和当前任务命中的 Skill。
2. 执行 `git status --short --branch`，确认不要覆盖 `history.md` 或 `.tmp/`。
3. 如果继续验证脚本，可先跑 `node "G:\浏览器下载\autoschedule-real-two-week.mjs" --help`。
4. 若要验证状态恢复，使用 `--state`；若要模拟本地页面从零排班，使用 `--zero --excel`。
5. 任何新的产品规则改动仍按 `AGENTS.md`：先确认业务例子、传播矩阵、红灯测试，再改实现；新的 bug 修复要更新 `history.md`。

## 旧快照（2026-09-02，仅供背景；以本文件上方最新快照为准）
