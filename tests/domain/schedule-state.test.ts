...
async function scheduledSingleWorkerState(
  ...
  state.assignments = (
    await generateSchedule(state, state.activeScheduleDate)
  ).assignments;
  return state;
}

describe("staff status schedule lifecycle", () => {
  it("removes a worker from the active schedule immediately after changing to leave", async () => {
    const state = await scheduledSingleWorkerState();
    const person = state.staff[0]!;
    expect(
  ...
  it("removes a worker from the active schedule immediately after changing to sick leave", async () => {
    const state = await scheduledSingleWorkerState();
    const person = state.staff[0]!;
    state.history = [
      {
  ...
  it("allows a worker to be scheduled again after returning to normal", async () => {
    const state = await scheduledSingleWorkerState("休假");
    const person = state.staff[0]!;
    expect(
      state.assignments.some((assignment) => assignment.staffId === person.id)
    ).toBe(false);
