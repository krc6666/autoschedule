...
import {
  ...
  recentHistory,
} from "../../src/domain/statistics/fatigue";
...
describe("fatigue domain", () => {
  it("uses only earlier records inside the history window", () => {
    ...
    expect(
      recentHistory(records, "2026-07-18", 7).map((item) => item.date)
    ).toEqual(["2026-07-17"]);
  });
  ...
  it("selects the latest archived workdays instead of calendar days", () => {
    ...
    expect(
      recentArchivedWorkdays(records, "2026-07-20", 2).map((item) => item.date)
    ).toEqual(["2026-07-14", "2026-07-18"]);
  });

  it("counts consecutive prior work days and applies the configured penalty", () => {
    const state = createDefaultState();
    const records = [
      record("2026-07-15", "2", 2),
      record("2026-07-16", "2", 2),
      record("2026-07-17", "2", 2),
    ];
