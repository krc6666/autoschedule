...
import { queryInternationalFlights } from "../../src/infrastructure/flight-query";
...
describe("online flight query", () => {
  it("queries the selected workday from 06:00 through next-day 06:00 and keeps international flights", async () => {
    ...
    expect(result.flights.map((flight) => flight.flightNo)).toEqual([
      "CX937",
      "TR121",
    ]);
    expect(result.flights[1]).toMatchObject({
      date: "2026-07-28",
      departureTime: "00:55",
    });
  });
  ...
  it("reports an invalid response instead of importing unknown data", async () => {
    ...
    await expect(
      queryInternationalFlights("2026-07-27", fetcher)
    ).rejects.toThrow("航班查询接口返回格式异常");
  });
});
