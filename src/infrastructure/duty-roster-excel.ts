...
import { getMonthlyDutyRoster, monthlyDutyDates } from "../domain/duty-roster";
...
function sameDutyCycle(referenceDate: string, date: string): boolean {
  ...
  return Number.isFinite(reference) && Number.isFinite(candidate)
    && Math.abs(Math.round((candidate - reference) / 86_400_000)) % 2 === 0;
}

function sheetRows(workbook: XLSX.WorkBook): Row[] | null {
  const sheetName = workbook.SheetNames.find((name) => name.includes("值班") && name.includes("备勤"))
    ?? workbook.SheetNames.find((name) => name.includes("值班") || name.includes("备勤"));
  const worksheet = sheetName ? workbook.Sheets[sheetName] : undefined;
  return worksheet ? XLSX.utils.sheet_to_json<Row>(worksheet, { header: 1, raw: false, defval: "" }) : null;
}
...
export function parseDutyRosterWorkbook(
  ...
  const rowsByDate = new Map<string, ImportedDutyRosterRow>(workDates.map((date) => [date, {
    date,
  ...
  if (!data) {
    return { month, referenceDate, rows: [...rowsByDate.values()], recognizedAssignments: 0, canApply: false, warnings, errors: ["未找到名称包含“值班”或“备勤”的工作表"] };
  }

  const headerCandidates = data.slice(0, 20).map((row, rowIndex) => ({
    rowIndex,
    dates: row.map((value, columnIndex) => ({ columnIndex, date: parseHeaderDate(value, month) })).filter((item): item is { columnIndex: number; date: string } => Boolean(item.date))
  }));
  const header = headerCandidates.sort((left, right) => right.dates.length - left.dates.length)[0];
  if (!header || header.dates.length < 2) {
    return { month, referenceDate, rows: [...rowsByDate.values()], recognizedAssignments: 0, canApply: false, warnings, errors: ["未识别到横向日期表头，请使用 MMDD、YYYY-MM-DD 或“月/日”格式"] };
  }
  ...
  const nameLocation = data.slice(0, header.rowIndex + 1).flatMap((row, rowIndex) => row.map((value, columnIndex) => ({ rowIndex, columnIndex, value: compact(value) })))
    .find((cell) => cell.value.includes("姓名"));
  const nameIndex = nameLocation?.columnIndex ?? Math.max(0, header.dates[0]!.columnIndex - 1);
  ...
  for (let rowIndex = header.rowIndex + 1; rowIndex < data.length; rowIndex += 1) {
    ...
    const assignments = header.dates.map(({ columnIndex, date }) => ({ date, kind: marker(row[columnIndex]) }))
      .filter((item) => item.kind === "duty" || item.kind === "standby");
    header.dates.forEach(({ columnIndex, date }) => {
      if (marker(row[columnIndex]) === "unknown") unknownMarkers.push(`${date}“${compact(row[columnIndex])}”`);
    });
    if (!assignments.length) continue;
    const name = compact(row[nameIndex]);
    const matches = staffByName.get(name) ?? [];
    if (!name || matches.length !== 1) {
      errors.push(matches.length > 1 ? `姓名“${name}”匹配到多名人员` : `未匹配人员“${name || `第${rowIndex + 1}行` }”`);
      continue;
    }
    ...
    for (const assignment of assignments) {
      const workDate = assignment.kind === "duty" ? assignment.date : addIsoDays(assignment.date, -1);
      if (!sameDutyCycle(referenceDate, workDate)) {
        errors.push(`${assignment.date}的${assignment.kind === "duty" ? "值班" : "备勤"}不属于当前班组 ${month} 的有效轮值日期`);
        continue;
      }
      ...
      if (assignment.kind === "duty") {
        if (!person.dutyQualified) {
          errors.push(`${person.name}不具备值班资质，不能安排 ${assignment.date} 值班`);
          continue;
        }
  ...
  for (const row of rowsByDate.values()) {
    if (row.dutyStaffId && row.standbyStaffIds.includes(row.dutyStaffId)) {
      ...
      errors.push(`${row.date}值班与${row.standbyDate}次日备勤不能由同一人${person ? `（${person.name}）` : ""}承担`);
    }
  }
  if (unknownMarkers.length) warnings.push(`忽略未识别标记：${unknownMarkers.slice(0, 5).join("、")}${unknownMarkers.length > 5 ? "等" : ""}`);
  const rowValues = [...rowsByDate.values()]
    .filter((row) => row.dutyIncluded || row.standbyIncluded)
    .sort((left, right) => left.date.localeCompare(right.date));
  const missingDuty = rowValues.filter((row) => row.dutyIncluded && !row.dutyStaffId).length;
  const missingStandby = rowValues.filter((row) => row.standbyIncluded && row.standbyStaffIds.filter(Boolean).length < 2).length;
  if (missingDuty) warnings.push(`本月 ${missingDuty} 个工作日未配置值班人员`);
  if (missingStandby) warnings.push(`本月 ${missingStandby} 个次日备勤未满两人`);
  return {
    month,
    referenceDate,
    rows: rowValues,
...
export async function importDutyRosterWorkbook(file: File, staff: Staff[], referenceDate: string): Promise<DutyRosterImportPreview> {
...
export function buildDutyRosterTemplateWorkbook(state: AppState, referenceDate: string): XLSX.WorkBook {
  ...
  const regularStaff = state.staff.filter((person) => person.staffType === "常规");
  const monthlyRoster = getMonthlyDutyRoster(state, referenceDate);
  const dutyStaffByDate = new Map(monthlyRoster.map((row) => [row.date, row.dutyStaffId]));
  const standbyStaffByDate = new Map(monthlyRoster.map((row) => [addIsoDays(row.date, 1), row.standbyStaffIds]));
  const data: unknown[][] = [
    [`国际值机${year}年${month}月排班、值班、备勤表（本组）`, ...Array.from({ length: dayCount + 1 }, () => "")],
    ["序号", "姓名", ...dates.map((day) => weekdays[new Date(Date.UTC(year, month - 1, day)).getUTCDay()])],
    ["", "", ...dates.map((day) => `${String(month).padStart(2, "0")}${String(day).padStart(2, "0")}`)],
    ...regularStaff.map((person, index) => [index + 1, person.name, ...dates.map((day) => {
  ...
  worksheet["!cols"] = [{ wch: 6 }, { wch: 12 }, ...dates.map(() => ({ wch: 6 }))];
  worksheet["!rows"] = [{ hpt: 28 }, { hpt: 22 }, { hpt: 22 }, ...regularStaff.map(() => ({ hpt: 24 }))];
  ...
  for (let row = range.s.r; row <= range.e.r; row += 1) {
    for (let column = range.s.c; column <= range.e.c; column += 1) {
      ...
      cell.s = {
        font: { bold: isTitle || isHeader || column < 2, sz: isTitle ? 15 : 10, color: { rgb: isDuty ? "C00000" : "1F2328" } },
        fill: { fgColor: { rgb: isStandby ? "F8CBAD" : isRest ? "EAF3F3" : isHeader ? "F2F2F2" : "FFFFFF" } },
