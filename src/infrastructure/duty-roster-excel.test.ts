...
import { buildDutyRosterTemplateWorkbook, parseDutyRosterWorkbook } from "./duty-roster-excel";
...
describe("duty roster workbook boundary", () => {
  it("parses 24 as duty and maps rest-day standby to the previous workday", () => {
    ...
    const preview = parseDutyRosterWorkbook(workbook, state.staff, "2026-07-01");
    ...
    expect(preview.rows.find((row) => row.date === "2026-07-03")?.dutyStaffId).toBe(fourth!.id);
  });
  ...
  it("exports a screenshot-like monthly template with workdays and rest days", () => {
    ...
    const data = XLSX.utils.sheet_to_json<unknown[]>(workbook.Sheets["值班备勤表"]!, { header: 1, raw: false, defval: "" });
    ...
    const cells = Object.values(workbook.Sheets["值班备勤表"]!).filter((cell): cell is XLSX.CellObject => Boolean(cell) && typeof cell === "object" && "v" in cell);
    expect(cells.find((cell) => cell.v === "24")?.s).toMatchObject({ font: { color: { rgb: "C00000" } } });
    expect(cells.find((cell) => cell.v === "备勤")?.s).toMatchObject({ fill: { fgColor: { rgb: "F8CBAD" } } });
  });
  ...
  it("maps a first-of-month standby back to the previous workday without covering its duty", () => {
    ...
    const preview = parseDutyRosterWorkbook(workbook, state.staff, "2026-08-02");
    const previousMonth = preview.rows.find((row) => row.date === "2026-07-31");

    expect(previousMonth).toMatchObject({ standbyDate: "2026-08-01", dutyIncluded: false, standbyIncluded: true });
    expect(previousMonth?.standbyStaffIds).toEqual([state.staff[0]!.id, state.staff[1]!.id]);
  });
});
