...
import {
  createBrowserScheduleRunController,
  type ScheduleRunController,
} from "./app/schedule-run-controller";
import {
  prepareWorkbookImport,
  type WorkbookImportMode,
} from "./app/workbook-import-controller";
import { applyDutyRosterImport } from "./app/workbook-actions";
...
import {
  queryInternationalFlights,
  type OnlineFlightQueryResult,
} from "./infrastructure/flight-query";
...
export class AutoScheduleApp {
  ...
  private scheduleDate =
    localStorage.getItem("autoschedule.scheduleDate") || todayIso();
  ...
  private scheduleZoom = Math.min(
    1.6,
    Math.max(
      0.7,
      Number(localStorage.getItem("autoschedule.scheduleZoom")) || 1
    )
  );
  private importMode: WorkbookImportMode = "all";
  ...
  private readonly scheduleRunner: ScheduleRunController;
  private readonly root: HTMLElement;

  constructor(root: HTMLElement) {
    this.root = root;
    this.scheduleRunner = createBrowserScheduleRunController(root);
  ...
  private view(): string {
    switch (this.activeSection) {
      case "config":
        return renderConfig(this.state);
      case "flights":
        return renderFlights(this.state);
      case "schedule":
        return renderSchedule(this.state, this.scheduleDate, {
          field: this.loadSortField,
          direction: this.loadSortDirection,
          zoom: this.scheduleZoom,
        });
      case "policy":
        return renderSchedulePolicy(this.state);
      case "statistics":
        return renderStatistics(this.state, this.scheduleDate);
      case "history":
        return renderHistory(this.state);
      default:
        return renderOverview(this.state, this.scheduleDate);
    }
  }

  private render(): void {
    this.loadDetailsOpen =
      this.root.querySelector<HTMLDetailsElement>(".load-details")?.open ??
      this.loadDetailsOpen;
    this.openDutyRosterSections = new Set(
      [
        ...this.root.querySelectorAll<HTMLDetailsElement>(
          ".duty-roster-details[open]"
        ),
      ]
        .map((element) => element.dataset.dutyRosterSection ?? "")
        .filter(Boolean)
    );
    this.openPositionFlights = new Set(
      [
        ...this.root.querySelectorAll<HTMLDetailsElement>(
          ".position-rule-group[open]"
        ),
      ]
        .map((element) => element.dataset.positionFlight ?? "")
        .filter(Boolean)
    );
    this.openPolicyCards = this.policySearchOpenCards
      ...
      : new Set(
          [
            ...this.root.querySelectorAll<HTMLDetailsElement>(
              ".policy-rule-card[open]"
            ),
          ]
            .map((element) => element.dataset.policyCard ?? "")
            .filter(Boolean)
        );
    this.policySearchOpenCards = null;
    this.openConfigSections = new Set(
      [
        ...this.root.querySelectorAll<HTMLDetailsElement>(
          ".config-collapsible[open]"
        ),
      ]
        .map((element) => element.dataset.configSection ?? "")
        .filter(Boolean)
    );
    ...
    this.root
      .querySelectorAll<HTMLDetailsElement>(".position-rule-group")
      .forEach((element) => {
        element.open = this.openPositionFlights.has(
          element.dataset.positionFlight ?? ""
        );
      });
    this.root
      .querySelectorAll<HTMLDetailsElement>(".policy-rule-card")
      .forEach((element) => {
        element.open = this.openPolicyCards.has(
          element.dataset.policyCard ?? ""
        );
      });
    this.root
      .querySelectorAll<HTMLDetailsElement>(".config-collapsible")
      .forEach((element) => {
        element.open = this.openConfigSections.has(
          element.dataset.configSection ?? ""
        );
      });
    const loadDetails =
      this.root.querySelector<HTMLDetailsElement>(".load-details");
    if (loadDetails) loadDetails.open = this.loadDetailsOpen;
    this.root
      .querySelectorAll<HTMLDetailsElement>(".duty-roster-details")
      .forEach((element) => {
        element.open = this.openDutyRosterSections.has(
          element.dataset.dutyRosterSection ?? ""
        );
      });
  }
  ...
  private openImport(mode: WorkbookImportMode): void {
  ...
  private handleDragStart(event: Event): void {
    const drag = (event.target as Element).closest<HTMLElement>(
      "[data-drag-staff], [data-drag-assignment]"
    );
  ...
  private handleDrop(event: Event): void {
    ...
    try {
      payload = JSON.parse(
        dataTransfer.getData("application/x-autoschedule") || "{}"
      );
    } catch {
      return;
    }
    ...
    if (!assignmentId || !payload.staffId) {
      const source = payload.assignmentId
        ? this.state.assignments.find(
            (item) => item.id === payload.assignmentId
          )
        : undefined;
      if (source?.staffId)
        this.assignStaff(assignmentId, source.staffId, source.id);
      return;
    }
  ...
  private handlePointerMove(event: PointerEvent): void {
    ...
    this.root
      .querySelectorAll(".schedule-cell.is-drop-target")
  ...
  private handlePointerUp(event: PointerEvent): void {
    ...
    const source = this.state.assignments.find(
      (assignment) => assignment.id === drag.assignmentId
    );
  ...
  private handleClick(event: Event): void {
    ...
    if (!action) return;
    if (isAppChangeAction(action)) return;
    if (!isAppClickAction(action)) {
      console.error(`未知界面操作：${action}`);
      return;
    }

    const actions: Record<AppClickAction, () => void> = {
      "generate-schedule": () => void this.generate(),
      ...
      "download-duty-roster-template": () =>
        void this.downloadDutyRosterTemplate(id),
      ...
      "apply-flight-plan-reconciliation": () =>
        this.applyOnlineFlightPlanReconciliation(),
      ...
      "sort-counters-desc": () =>
        this.sortCountersDescending(target.dataset.flightNo ?? ""),
      ...
      "add-late-shift-recovery-position": () =>
        this.addLateShiftRecoveryPosition(),
      "delete-late-shift-recovery-position": () =>
        this.deleteLateShiftRecoveryPosition(id),
      ...
      "archive-and-next-duty": () => void this.archiveAndScheduleNextDutyDay(),
  ...
  private handleInput(event: Event): void {
    ...
    if (target.id === "policy-rule-search")
      this.filterPolicyRules(target.value);
  }
  ...
  private filterPolicyRules(value: string): void {
    ...
    const cards = [
      ...this.root.querySelectorAll<HTMLDetailsElement>(
        ".schedule-policy-section > .policy-rule-card"
      ),
    ];
    const allCards = [
      ...this.root.querySelectorAll<HTMLDetailsElement>(
    ...
    if (query && !this.policySearchOpenCards) {
      this.policySearchOpenCards = new Set(
        allCards
          .filter((card) => card.open)
          .map((card) => card.dataset.policyCard ?? "")
      );
    }
    ...
    cards.forEach((card) => {
      const summaryMatches =
        Boolean(query) &&
        searchableText(
          card.querySelector<HTMLElement>(":scope > summary") ?? card
        ).includes(query);
      card.hidden = Boolean(query) && !searchableText(card).includes(query);
      if (query && !card.hidden) card.open = true;
      card
        .querySelectorAll<HTMLDetailsElement>(".policy-transition-card")
        .forEach((nested) => {
          nested.hidden =
            Boolean(query) &&
            !summaryMatches &&
            !searchableText(nested).includes(query);
          if (query && !nested.hidden) nested.open = true;
        });
      card
        .querySelectorAll<HTMLTableRowElement>(".policy-ledger-table tbody tr")
        .forEach((row) => {
          row.hidden =
            Boolean(query) &&
            !summaryMatches &&
    ...
    if (!query && this.policySearchOpenCards) {
      cards.forEach((card) => {
        ...
        card
          .querySelectorAll<HTMLElement>(
            ".policy-transition-card, .policy-ledger-table tbody tr"
          )
          .forEach((element) => {
      ...
      allCards.forEach((card) => {
        card.open =
          this.policySearchOpenCards?.has(card.dataset.policyCard ?? "") ??
          false;
      });
      this.policySearchOpenCards = null;
    }
  ...
  private async handleChange(event: Event): Promise<void> {
    ...
    if (target.id === "schedule-date") {
      ...
      if (
        ...
        clearActiveSchedule(this.state);
        this.state = saveState(this.state);
      }
    ...
    const action = target.dataset.action;
    if (action && !isAppChangeAction(action)) {
      console.error(`未知界面变更操作：${action}`);
      return;
    }
    ...
    if (
      action === "toggle-admin-support-mode" &&
      target instanceof HTMLInputElement
    ) {
      await this.toggleAdministrativeSupport(target.checked);
      return;
    }
    if (action === "load-sort-field") {
      ...
      if (
        ...
        this.loadSortField = field;
      this.render();
      return;
    }
    if (action === "load-sort-direction") {
      this.loadSortDirection = target.value === "asc" ? "asc" : "desc";
      this.render();
      return;
    }
    if (action === "create-temporary-assignment") {
  ...
  private async generate(): Promise<void> {
    try {
      const result = await this.scheduleRunner.calculate(
        this.state,
        this.scheduleDate
      );
      installGeneratedSchedule(this.state, this.scheduleDate, result);
      this.activeSection = "schedule";
      this.commit(
        result.unfilledCount
          ? `排班已生成，${result.unfilledCount} 个岗位待补位`
          : "排班已完整生成"
      );
    } catch (error) {
      this.scheduleRunner.hideProgress();
      this.toast(
        `排班失败：${error instanceof Error ? error.message : String(error)}`,
        "danger"
      );
    }
  }
  ...
  private async exportPng(): Promise<void> {
    if (!this.confirmStaleSchedule("导出当前排班图片")) return;
    try {
  ...
  private async exportSchedule(): Promise<void> {
    if (!this.confirmStaleSchedule("导出当前排班结果")) return;
    const { buildScheduleWorkbook, writeWorkbook } =
      await import("./infrastructure/excel");
    writeWorkbook(
      buildScheduleWorkbook(this.state.assignments, this.scheduleDate),
      `排班表_${this.scheduleDate}.xlsx`
    );
  }

  private async exportHtml(): Promise<void> {
    if (!this.confirmStaleSchedule("导出当前 HTML 排班页")) return;
  ...
  private showOnlineFlightQuery(): void {
    this.modal(
      "在线查询航班",
      renderOnlineFlightQuery(
        this.state,
        this.scheduleDate,
        this.onlineFlightQueryDate,
        this.onlineFlightQueryResult
      ),
  ...
  private async runOnlineFlightQuery(): Promise<void> {
    const dateInput = assertElement<HTMLInputElement>(
      "#online-flight-query-date"
    );
    this.onlineFlightQueryDate = dateInput.value;
    const button = this.root.querySelector<HTMLButtonElement>(
      '[data-action="run-online-flight-query"]'
    );
    if (button) {
    ...
    try {
      this.onlineFlightQueryResult = await queryInternationalFlights(
        this.onlineFlightQueryDate
      );
      this.showOnlineFlightQuery();
    } catch (error) {
      this.toast(
        `在线航班查询失败：${error instanceof Error ? error.message : String(error)}`,
        "danger"
      );
      if (button) {
  ...
  private applyOnlineFlightPlanReconciliation(): void {
    ...
    const templateIds = [
      ...this.root.querySelectorAll<HTMLInputElement>(
        'input[name="online-flight-addition"]:checked'
      ),
    ]
    ...
    const flightIds = [
      ...this.root.querySelectorAll<HTMLInputElement>(
        'input[name="online-flight-removal"]:checked'
      ),
    ]
      .map((input) => input.value)
      .filter(Boolean);
    ...
    if (flightIds.length) {
      ...
      if (
        !confirm(
          `确认从当前航班计划删除 ${flightIds.length} 个航班（${flightNos}）？删除后已有排班结果将失效，需重新排班。`
        )
      )
        return;
    }
    ...
    this.closeModal();
    this.commit(
      `已更新 ${this.scheduleDate} 航班计划：新增 ${result.added}、删除 ${result.removed}、保留 ${reconciliation.retained.length}${result.skipped ? `，跳过 ${result.skipped} 项` : ""}`
    );
  }

  private async toggleAdministrativeSupport(enabled: boolean): Promise<void> {
    const previous = this.state.settings.adminSupportEnabled;
    ...
    try {
      ...
      installGeneratedSchedule(this.state, this.scheduleDate, result);
      this.commit(
        enabled ? "行政支援模式已启用，行政岗位已留空" : "行政支援模式已关闭"
      );
    } catch (error) {
      this.state.settings.adminSupportEnabled = previous;
      this.render();
      this.toast(
        `行政支援模式切换失败：${error instanceof Error ? error.message : String(error)}`,
        "danger"
      );
    }
  }
  ...
  private setScheduleZoom(value: number): void {
    ...
    localStorage.setItem(
      "autoschedule.scheduleZoom",
      String(this.scheduleZoom)
    );
    this.render();
  }

  private saveSchedulePolicy(): void {
    const readNumber = (selector: string): number =>
      Number(assertElement<HTMLInputElement>(selector).value);
    const stale = applySchedulePolicy(this.state, {
      highLoadProtectionEnabled:
        assertElement<HTMLInputElement>("#policy-enabled").checked,
      highLoadFatigueThreshold: readNumber("#policy-fatigue-threshold"),
      highLoadRecoveryMinutes: readNumber("#policy-recovery-minutes"),
      remarkedPositionHighLoad: assertElement<HTMLInputElement>(
        "#policy-remarked-high-load"
      ).checked,
      rollingLoadProtectionEnabled: assertElement<HTMLInputElement>(
        "#policy-rolling-load-enabled"
      ).checked,
      ...
      positionRotationEnabled: assertElement<HTMLInputElement>(
        "#policy-rotation-enabled"
      ).checked,
      nextDutyRestProtectionEnabled: assertElement<HTMLInputElement>(
        "#policy-next-duty-rest-enabled"
      ).checked,
      lateShiftRecoveryEnabled: assertElement<HTMLInputElement>(
        "#policy-late-shift-recovery-enabled"
      ).checked,
      lateShiftStartTime: assertElement<HTMLInputElement>(
        "#policy-late-shift-start-time"
      ).value,
      lateShiftLatestWindowMinutes: readNumber(
        "#policy-late-shift-latest-window"
      ),
      teamLeaderConcurrentSupervisionMaxOverlapMinutes: readNumber(
        "#policy-team-leader-concurrent-overlap"
      ),
      workloadBalanceEnabled: assertElement<HTMLInputElement>(
        "#policy-workload-balance-enabled"
      ).checked,
      maxWorkHoursDifference: readNumber("#policy-max-work-hours-difference"),
      maxTodayFatigueDifference: readNumber(
        "#policy-max-today-fatigue-difference"
      ),
      dutyFatiguePoints: readNumber("#policy-duty-fatigue-points"),
      earlyDepartureCutoffTime: assertElement<HTMLInputElement>(
        "#policy-early-departure-cutoff"
      ).value,
      afternoonRestStartTime: assertElement<HTMLInputElement>(
        "#policy-afternoon-rest-start"
      ).value,
      afternoonRestEndTime: assertElement<HTMLInputElement>(
        "#policy-afternoon-rest-end"
      ).value,
    });
    this.commit(
      stale
  ...
  private deleteDutyPriority(id: string): void {
    const priority = this.state.settings.dutyPositionPriorities.find(
      (item) => item.id === id
    );
    if (
      !priority ||
      !confirm(
        `确认删除 ${priority.flightNo || "未填写航班"} / ${priority.positionKeyword || "任意岗位"}？`
      )
    )
      return;
  ...
  private addLateShiftRecoveryPosition(): void {
    addLateShiftRecoveryPositionRule(this.state);
    this.openPolicyCards.add("late-shift-recovery");
    this.commit("已新增末班重点岗位规则");
  }

  private deleteLateShiftRecoveryPosition(id: string): void {
    const rule = this.state.settings.lateShiftRecoveryPositionRules.find(
      (item) => item.id === id
    );
    if (
      !rule ||
      !confirm(`确认删除末班重点岗位规则“${rule.keyword || "未填写关键词"}”？`)
    )
      return;
    if (!deleteLateShiftRecoveryPositionRule(this.state, id)) return;
    this.openPolicyCards.add("late-shift-recovery");
    this.commit("末班重点岗位规则已删除");
  }

  private deleteRecoveryTarget(id: string): void {
    const target = this.state.settings.nextWorkdayRecoveryTargets.find(
      (item) => item.id === id
    );
    if (
      !target ||
      !confirm(
        `确认删除 ${target.flightNo || "未填写航班"} / ${target.positionKeyword || "未填写关键词"}？`
      )
    )
      return;
  ...
  private deleteSupervisorCoverageRule(id: string): void {
    const rule = this.state.settings.mobileSupervisorCoverageRules.find(
      (item) => item.id === id
    );
    if (
      !rule ||
      !confirm(`确认删除机动督导兼任规则“${rule.keyword || "未填写关键词"}”？`)
    )
      return;
  ...
  private showTemplates(): void {
    this.modal(
      ...
      this.state.templates.length
        ? `<div class="list-group">${this.state.templates.map((template) => `<button class="list-group-item list-group-item-action d-flex justify-content-between align-items-center" type="button" data-action="select-template" data-id="${template.id}"><span><strong>${escapeHtml(template.flightNo)}</strong><small class="text-secondary ms-3">${escapeHtml(template.startTime)}–${escapeHtml(template.endTime)}</small></span><span class="badge text-bg-light">${template.positions.length} 岗</span></button>`).join("")}</div>`
        : `<div class="empty-state">尚无航班模板</div>`,
      `<button class="btn btn-secondary" type="button" data-bs-dismiss="modal">关闭</button>`
    );
  }
  ...
  private deleteStaff(id: string): void {
    ...
    if (
      !person ||
      !confirm(`确认删除 ${person.name}？相关岗位资质也会同步移除。`)
    )
      return;
  ...
  private addPositions(): void {
    ...
    const requestedCount = Number(
      assertElement<HTMLInputElement>("#position-batch-count").value
    );
    if (!flightNo) {
      this.toast("请先配置航班", "warning");
      return;
    }
  ...
  private createTemporaryAssignment(
    ...
    const position =
      normalizeText(
        slot.querySelector<HTMLInputElement>('[data-empty-field="position"]')
          ?.value
      ) || "临时岗位";
    const staffName = normalizeText(
      slot.querySelector<HTMLInputElement>('[data-empty-field="staffName"]')
        ?.value
    );
    if (
      !createTemporaryAssignmentAction(
        this.state,
        flightId,
        position,
        staffName,
  ...
  private showQualified(id: string): void {
    ...
    this.modal(
      `${rule.flightNo} / ${rule.name} 资质`,
      body,
      `<button class="btn btn-secondary" type="button" data-bs-dismiss="modal">取消</button><button class="btn btn-primary" type="button" data-action="save-qualified" data-id="${id}">保存</button>`
    );
  }

  private setQualifiedSelection(checked: boolean): void {
    document
      .querySelectorAll<HTMLInputElement>('input[name="qualified-staff"]')
      .forEach((input) => {
  ...
  private saveQualified(id: string): void {
    ...
    const staffIds = [
      ...document.querySelectorAll<HTMLInputElement>(
        'input[name="qualified-staff"]:checked'
      ),
    ].map((input) => input.value);
  ...
  private assignStaff(
    ...
    if (!this.confirmLateShiftCutoffOverride(assignmentId, staffId)) return;
    const result = editScheduleAssignment(
      this.state,
      assignmentId,
      staffId,
      sourceAssignmentId
  ...
  private clearSchedule(): void {
    if (!this.state.assignments.length || !confirm("确认清空当前排班？"))
      return;
    clearActiveSchedule(this.state);
    this.commit("当前排班已清空");
  }
  ...
  private archiveSchedule(): void {
    ...
    if (!records.length) {
      this.toast("没有可归档的已排岗位", "warning");
      return;
    }
    if (!this.confirmStaleSchedule("归档当前排班")) return;
    if (
      !confirm(
        `将 ${records.length} 条已排岗位归档到 ${this.scheduleDate}？同日旧记录会被替换。`
      )
    )
      return;
  ...
  private rebalanceDutyRosterMonth(date: string): void {
    if (
      !confirm(
        `清除 ${date.slice(0, 7)} 的人工轮值调整，并按值班优先规则重新均衡？`
      )
    )
      return;
  ...
  private confirmLateShiftCutoffOverride(
    assignmentId: string,
    staffId: string
  ): boolean {
  ...
  private async downloadDutyRosterTemplate(date: string): Promise<void> {
    const { buildDutyRosterTemplateWorkbook } =
    ...
    writeWorkbook(
      buildDutyRosterTemplateWorkbook(this.state, referenceDate),
      `值班备勤表_${referenceDate.slice(0, 7)}.xlsx`
    );
  }
  ...
  private applyPendingDutyRosterImport(): void {
    ...
    this.closeModal();
    this.commit(
      `已导入 ${preview.month} 值班备勤表：${result.importedDays} 个工作班、${result.importedAssignments} 项安排；CX航前保持不变`
    );
  }
  ...
  private async archiveAndScheduleNextDutyDay(): Promise<void> {
    const records = this.currentScheduleHistory();
    if (!records.length) {
      this.toast("没有可归档的已排岗位", "warning");
      return;
    ...
    if (!this.confirmStaleSchedule("归档当前排班并生成后天排班")) return;
    if (
      !confirm(
    ...
    try {
      result = await this.scheduleRunner.calculate(this.state, nextDate);
    } catch (error) {
      this.scheduleRunner.hideProgress();
      this.toast(
    ...
    installGeneratedSchedule(this.state, nextDate, result);
    this.activeSection = "schedule";
    this.commit(
      result.unfilledCount
  ...
  private clearHistory(): void {
    if (!this.state.history.length || !confirm("确认清空全部历史排班？"))
      return;
  ...
  private async loadWorkbook(file: File): Promise<void> {
    try {
      ...
      const prepared = await prepareWorkbookImport(
        ...
        file,
        mode,
        this.dutyRosterImportReferenceDate
      );
      this.importMode = "all";
      if (prepared.kind === "duty-roster") {
        this.pendingDutyRosterImport = prepared.preview;
        this.modal(
          "值班备勤表导入预览",
          renderDutyRosterImportPreview(this.state, prepared.preview),
          `<button class="btn btn-secondary" type="button" data-bs-dismiss="modal">取消</button><button class="btn btn-primary" type="button" data-action="apply-duty-roster-import" ${prepared.preview.canApply ? "" : "disabled"}>确认应用预览中的值班备勤</button>`
        );
        return;
      }
      this.commit(
      ...
      if (prepared.recognized && prepared.warnings.length)
        this.toast(prepared.warnings.join("；"), "warning");
    } catch (error) {
      this.importMode = "all";
      this.toast(
  ...
  private updateField(
    ...
    if (entity === "duty-roster") {
      this.updateDutyRoster(
        id,
        input.dataset.dutySlot as DutyRosterSlot,
        String(value)
      );
      return;
    }
    if (entity === "assignment") {
      if (field === "staffName") {
        const person = this.state.staff.find(
          (item) => item.name === normalizeText(value)
        );
        if (person && !this.confirmLateShiftCutoffOverride(id, person.id)) {
          this.render();
          return;
        }
      }
    ...
    const policyUpdate = updatePolicyEntityField(
      this.state,
      entity,
      id,
      field,
    ...
    if (policyUpdate !== "not-policy") {
      if (policyUpdate === "missing") return;
      this.state = saveState(this.state);
      return;
    }
    const result = updateConfigurationField(
      this.state,
      entity,
      id,
      field,
  ...
  private confirmStaleSchedule(action: string): boolean {
    return (
      !this.state.schedulePolicyStale ||
      confirm(`排班规则已更新，但当前班表尚未重新生成。仍要${action}吗？`)
    );
  }
}
