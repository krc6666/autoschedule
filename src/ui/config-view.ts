  ...
  return `
    ...
    <section class="workspace-section">
      ...
      <div class="position-rule-groups">${positionGroups.map(({ flightNo, rules }) => {
        ...
        const supportCount = rules.filter((rule) => rule.category === "支援").length;
        ...
        return `<details class="position-rule-group" data-position-flight="${escapeHtml(flightNo)}"><summary><strong>${escapeHtml(flightNo)}</strong><span>${regularCount} 常规 · ${supportCount} 支援 · ${diversionCount} 分流 · ${adminSupportCount} 行政支援</span><i class="bi bi-chevron-down"></i></summary><div class="position-group-toolbar"><button class="btn btn-sm btn-outline-secondary" type="button" data-action="sort-counters-desc" data-flight-no="${escapeHtml(flightNo)}"><i class="bi bi-sort-numeric-down-alt me-1"></i>柜台从大到小</button></div><div class="table-responsive"><table class="table align-middle data-table position-rule-table"><thead><tr><th>顺序</th><th>航班</th><th>岗位</th><th>分类</th><th>疲劳点</th><th>启用旅客人数</th><th>提前撤岗</th><th>资质人员</th><th>备注</th><th class="action-col"><span class="visually-hidden">操作</span></th></tr></thead><tbody>${rules.map((rule, ruleIndex) => `<tr>
          ...
          <td><select class="form-select form-select-sm" data-entity="position" data-id="${rule.id}" data-field="category" aria-label="分类"><option ${rule.category === "常规" ? "selected" : ""}>常规</option><option ${rule.category === "支援" ? "selected" : ""}>支援</option><option ${rule.category === "分流" ? "selected" : ""}>分流</option><option ${rule.category === "行政支援" ? "selected" : ""}>行政支援</option></select></td>
