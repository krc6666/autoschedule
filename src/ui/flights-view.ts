...
export function renderFlights(state: AppState): string {
  return `<section class="workspace-section">
    <div class="section-heading"><div><h3>航班计划</h3><span>按开始时间参与自动排班</span></div><div class="d-flex gap-2"><button class="btn btn-outline-secondary" type="button" data-action="add-from-template"><i class="bi bi-copy me-2"></i>从模板添加</button><button class="btn btn-primary" type="button" data-action="add-flight"><i class="bi bi-plus-lg me-2"></i>新增航班</button></div></div>
    <div class="table-responsive">
      <table class="table align-middle data-table"><thead><tr><th>航班号</th><th>时间</th><th>预定人数</th><th>岗位</th><th>备注</th><th class="action-col"><span class="visually-hidden">操作</span></th></tr></thead><tbody>
        ${state.flights.length ? [...state.flights].sort((a, b) => a.startTime.localeCompare(b.startTime)).map((flight) => `<tr>
          <td><input class="form-control form-control-sm code-input" value="${escapeHtml(flight.flightNo)}" data-entity="flight" data-id="${flight.id}" data-field="flightNo" aria-label="航班号"></td>
