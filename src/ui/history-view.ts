...
export function renderHistory(state: AppState): string {
  ...
  return `<section class="workspace-section">
    <div class="section-heading"><div><h3>历史排班</h3><span>${dates.length} 个工作日 · ${state.history.length} 条记录</span></div><div class="d-flex gap-2"><button class="btn btn-outline-secondary" type="button" data-action="import-workbook"><i class="bi bi-file-earmark-arrow-up me-2"></i>导入历史</button><button class="btn btn-outline-danger" type="button" data-action="clear-history"><i class="bi bi-trash3 me-2"></i>清空历史</button></div></div>
