...
describe("scheduler domain", () => {
  it("keeps the next workday duty worker on an ordinary position when a safe priority-position alternative exists", () => {
    ...
    const result = generateSchedule(state, "2026-08-14");

    expect(
      result.assignments.find((item) => item.positionRuleId === "priority")
        ?.staffId
    ).toBe(alternate!.id);
    expect(
      result.assignments.find((item) => item.positionRuleId === "ordinary")
        ?.staffId
    ).toBe(protectedWorker!.id);
  });

  it("preserves a priority position when the next workday duty worker is the only qualified person and records the override", () => {
    const state = createDefaultState();
    ...
    expect(assignment.decisionTrace).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          outcome: "fallback",
          message: expect.stringContaining("唯一合格人员"),
        }),
      ])
    );
  ...
  it("uses a qualified team leader to cover two supervisors with a short overlap when that removes a regular vacancy", () => {
    const state = createDefaultState();
    ...
    const result = generateSchedule(state, "2026-07-29");
  ...
  it("keeps a regular vacancy when the supervisor overlap exceeds the configured team-leader limit", () => {
    const state = createDefaultState();
    const [leader, releasedWorker, dutyWorker] = state.staff
      .filter((person) => person.status === "正常")
      .slice(0, 3);
  ...
  it("does not use concurrent supervision when all regular positions are already filled", () => {
    const state = createDefaultState();
  ...
  it("does not grant concurrent-supervision permission to an ordinary worker", () => {
    const state = createDefaultState();
  ...
  it("does not use KE166 as either side of a concurrent-supervision pair", () => {
    const state = createDefaultState();
  ...
  it("uses original flight intervals for a diversion supervisor and safely backfills a three-flight chain", () => {
    const state = createDefaultState();
  ...
  it("does not use monthly frequency to choose an ordinary position worker", () => {
    ...
    expect(generateSchedule(state, "2026-08-23").assignments[0]?.staffId).toBe(
      first!.id
    );
  });
  ...
  it("prefers a rested worker for a high-load position during the recovery window", () => {
    ...
    expect(
      generateSchedule(state, "2026-07-18").assignments.find(
  ...
  it("uses the same-position frequency rule only while same-position fairness is enabled", () => {
    ...
    expect(generateSchedule(state, "2026-07-18").assignments[0]!.staffId).toBe(
      second!.id
    );
    state.settings.positionRotationEnabled = false;
    expect(generateSchedule(state, "2026-07-18").assignments[0]!.staffId).toBe(
  ...
  it("reduces the next late-shift load after a high-load final flight on the previous day", () => {
    ...
    expect(generateSchedule(state, "2026-07-20").assignments[0]!.staffId).toBe(
      restedWorker!.id
    );
  ...
  it("protects a final-late priority-position worker from a configured next-workday priority position before frequency balancing", () => {
    const state = createDefaultState();
    const [protectedWorker, alternate] = state.staff
      .filter((person) => person.status === "正常")
      .slice(0, 2);
  ...
  it("moves a previous final-late supervisor away from the next final-late supervisor position when a safe replacement exists", () => {
    const state = createDefaultState();
    ...
    const assignment = generateSchedule(state, "2026-08-23").assignments[0]!;
  ...
  it("keeps a previous final-late priority worker off flights starting at or after the configured next-workday cutoff", () => {
    const state = createDefaultState();
    const [protectedWorker, alternate] = state.staff
      .filter((person) => person.status === "正常")
      .slice(0, 2);
    ...
    const result = generateSchedule(state, "2026-08-23");
  ...
  it("keeps the KE166 mobile supervisor independent and lets another worker satisfy next-workday recovery", () => {
    const state = createDefaultState();
  ...
  it("keeps administrative support in its position role instead of moving the category to the bottom", () => {
    ...
    expect(
      generateSchedule(state, "2026-07-18").assignments.map(
        (item) => item.position
      )
    ).toEqual(["督导", "G14", "G13"]);
  });
  ...
  it("keeps administrative support positions empty even when basic positions are full", () => {
    ...
    const adminSupport = generateSchedule(state, "2026-07-18").assignments.find(
      (item) => item.position === "超规行李引导"
    );
  ...
  it("does not change regular supervisor selection when a worker is marked as team leader", () => {
    const scheduleSupervisor = (teamLeaderId?: string) => {
      ...
      return generateSchedule(state, "2026-07-18").assignments.find(
        (item) => item.positionRuleId === "supervisor"
      )!.staffId;
    };
  ...
  it("prefers another KE166 mobile supervisor when the first qualified worker is on duty next workday", () => {
    const state = createDefaultState();
    const [protectedWorker, alternate] = state.staff
      .filter((person) => person.status === "正常")
      .slice(0, 2);
    ...
    const assignments = generateSchedule(state, "2026-08-14").assignments;
  ...
  it("rotates the independent KE166 mobile supervisor after the previous workday", () => {
    const state = createDefaultState();
  ...
  it("keeps the only KE166 mobile supervisor pair complete and reports the consecutive exception", () => {
    const state = createDefaultState();
  ...
  it("keeps the duty identity but moves the worker to another configured priority position after a repeated duty target", () => {
    const state = createDefaultState();
    const [dutyWorker, alternate] = state.staff
      .filter((person) => person.status === "正常")
      .slice(0, 2);
  ...
  it("assigns the duty person to both a morning flight and the protected late position", () => {
    const state = createDefaultState();
    state.staff = state.staff.slice(0, 6);
    state.staff.forEach((person) => {
      person.dutyQualified = true;
    ...
    const dutyAssignments = generateSchedule(
      state,
      "2026-07-20"
  ...
  it("allows the duty morning assignment to use a flight starting after 08:30 and before noon", () => {
    const state = createDefaultState();
    ...
    const assignments = generateSchedule(state, "2026-08-11").assignments;
  ...
  it("moves the duty worker between morning flights to prevent a repeated priority position", () => {
    const state = createDefaultState();
    const [dutyWorker, otherWorker] = state.staff
      .filter((person) => person.status === "正常")
      .slice(0, 2);
    ...
    const result = generateSchedule(state, "2026-08-11");
  ...
  it("allows the duty person to fill a qualified middle position after both duty targets are locked", () => {
    const state = createDefaultState();
    ...
    const assignments = generateSchedule(state, "2026-07-29").assignments;
  ...
  it("breaks a strict transition rule before noon and records the override", () => {
    ...
    const target = generateSchedule(state, "2026-07-18").assignments.find(
  ...
  it("records a concrete staffing-shortage reason for an unfilled regular position before noon", () => {
    ...
    const unfilled = generateSchedule(state, "2026-07-18").assignments.find(
  ...
  it("keeps the duty position lock ahead of same-position rotation", () => {
    ...
    expect(generateSchedule(state, "2026-07-20").assignments[0]).toMatchObject({
      staffId: duty!.id,
      position: "H02",
    });
  });
  ...
  it("uses generation-stage consecutive priority-position protection before frequency review", () => {
    const state = createDefaultState();
  ...
  it("uses a targeted overlapping-flight chain to relieve a repeated high-fatigue priority position", () => {
    const state = createDefaultState();
  ...
  it("rotates KE166 counters before binding the mobile supervisor", () => {
    const state = createDefaultState();
  ...
  it("uses an idle qualified KE166 supervisor independently before any counter coverage", () => {
    const state = createDefaultState();
    const [
      repeatedH02,
      h02Replacement,
    ...
    const result = generateSchedule(state, "2026-10-05");
    const supervisor = result.assignments.find(
      (item) => item.positionRuleId === "ke-supervisor"
    )!;
    const h02 = result.assignments.find(
  ...
  it("chooses a non-repeated KE166 mobile supervisor after keeping final counter staff in place", () => {
    const state = createDefaultState();
  ...
  it("prefers a non-repeated KE166 counter supervisor over a repeated idle independent supervisor", () => {
    const state = createDefaultState();
  ...
  it("releases a non-repeated KE166 supervisor from an overlapping flight before retaining a repeated independent supervisor", () => {
    const state = createDefaultState();
  ...
  it("uses a multi-flight personnel chain to place a non-repeated KE166 counter supervisor", () => {
    const state = createDefaultState();
    const [
      repeatedSupervisor,
  ...
  it("uses a four-person open reassignment chain when swaps and closed cycles cannot rotate a repeated priority position", () => {
    const state = createDefaultState();
    const [
      repeatedWorker,
      firstMover,
  ...
  it("reports an unresolved second consecutive priority-position assignment", () => {
    const state = createDefaultState();
  ...
  it("prevents a second consecutive priority position before ordinary assignments consume a safe non-overlapping replacement", () => {
    const state = createDefaultState();
  ...
  it("uses previous-workday load recovery before monthly priority-position frequency", () => {
    const state = createDefaultState();
    ...
    ).toBe(lighterPreviousWorker!.id);
    state.assignments = result.assignments;
    const feedback = buildScheduleFeedback(state, "2026-10-24").find(
      (item) => item.key === "cross-workday-load"
    )!;
    expect(feedback).toMatchObject({ status: "已执行" });
    expect(feedback.text).toContain("跨工作班负荷互补");
  });

  it("keeps an unavoidable late-flight worker off an earlier priority position when a safe alternate exists", () => {
    const state = createDefaultState();
  ...
  it("blocks a strict transition after noon while prefer mode remains a fallback preference", () => {
    ...
    const preferredResult = generateSchedule(
      buildState("prefer"),
      "2026-07-18"
    );
    expect(
