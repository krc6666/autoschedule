...
export interface FlightPlanReconciliation<TFlight extends QueriedFlightReference = QueriedFlightReference> {
  onlineFlights: Array<{
    flight: TFlight;
...
export function buildFlightPlanReconciliation<TFlight extends QueriedFlightReference>(
  ...
  state.flights.forEach((flight) => currentByFlightNo.set(normalizedFlightNo(flight.flightNo), flight));
  const templateByFlightNo = new Map<string, FlightTemplate>();
  state.templates.forEach((template) => templateByFlightNo.set(normalizedFlightNo(template.flightNo), template));
  ...
  const retained = onlineFlights
    .filter((item): item is typeof item & { currentFlight: Flight } => Boolean(item.currentFlight))
    .map((item) => ({ flight: item.currentFlight, onlineFlight: item.flight }));
  const additions = onlineFlights
    .filter((item): item is typeof item & { template: FlightTemplate } => Boolean(item.template) && !item.currentFlight)
    .map((item) => ({ flight: item.flight, template: item.template }));
  const unmatched = onlineFlights.filter((item) => !item.template && !item.currentFlight).map((item) => item.flight);
  const removals = state.flights.filter((flight) => !seenOnlineFlightNos.has(normalizedFlightNo(flight.flightNo)));
  ...
  if (result.date !== currentScheduleDate) {
    ...
    removalBlockedReason = "查询结果为空，无法确认线上数据是否完整，不能批量删减。";
  }
