...
import { buildMonthlyPositionStatistics } from "./monthly-position-statistics";

const target = { flightNo: "TR121", position: "H02" } as const;

function historyRecord(id: string, date: string, flightNo: string, position: string, staffId: string, staffName: string): HistoryRecord {
  return {
    id,
    date,
    flightNo,
...
function assignment(id: string, ruleId: string, staffId: string, staffName: string): Assignment {
  return {
    id,
...
describe("monthly position statistics", () => {
  it("lists only current normal regular qualified staff, including people with zero assignments", () => {
    ...
    const [firstId, secondId, sickId, adminId] = rule.qualifiedStaffIds.slice(0, 4);
    ...
    state.history = [
      historyRecord("first", "2026-07-02", "TR121", "H02", first.id, first.name),
      historyRecord("duplicate", "2026-07-02", "TR121", "H02", first.id, first.name),
      historyRecord("other-flight", "2026-07-04", "OTHER", "H02", second.id, second.name),
      historyRecord("other-position", "2026-07-06", "TR121", "H03", second.id, second.name),
      historyRecord("other-month", "2026-06-30", "TR121", "H02", second.id, second.name),
      historyRecord("sick", "2026-07-08", "TR121", "H02", sick.id, sick.name),
      historyRecord("admin", "2026-07-10", "TR121", "H02", admin.id, admin.name)
    ];

    const result = buildMonthlyPositionStatistics(state, "2026-07-18", target);

    expect(result.rows.map((row) => ({ id: row.staff.id, count: row.dates.length, dates: row.dates }))).toEqual([
  ...
  it("uses the current final schedule instead of same-day history and reflects a manual staff change", () => {
    ...
    state.history = [
      historyRecord("old-day", "2026-07-16", "TR121", "H02", archived.id, archived.name),
      historyRecord("stale-same-day", "2026-07-18", "TR121", "H02", archived.id, archived.name)
    ];
    state.activeScheduleDate = "2026-07-18";
    state.assignments = [assignment("current", rule.id, current.id, current.name)];

    const result = buildMonthlyPositionStatistics(state, "2026-07-18", target);

    expect(result.rows.find((row) => row.staff.id === archived.id)?.dates).toEqual(["2026-07-16"]);
    expect(result.rows.find((row) => row.staff.id === current.id)?.dates).toEqual(["2026-07-18"]);
  });
  ...
  it("reports whether the current regular position configuration has eligible staff", () => {
    ...
    expect(buildMonthlyPositionStatistics(state, "2026-07-18", target)).toMatchObject({ configured: false, rows: [] });
    ...
    expect(buildMonthlyPositionStatistics(emptyState, "2026-07-18", target)).toMatchObject({ configured: true, rows: [] });
  });
});
