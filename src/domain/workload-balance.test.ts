...
import { buildWorkloadTasks, evaluateWorkloadBalance } from "./workload-balance";
...
describe("workload balance task facts", () => {
  it("does not count administrative support placeholders as automatic workload", () => {
    ...
    const regular = state.positionRules.find((rule) => rule.flightNo === flight.flightNo && rule.category === "常规")!;
