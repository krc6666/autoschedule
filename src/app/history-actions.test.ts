...
describe("history actions", () => {
  ...
  it("preserves the priority remark through archive and protects the worker on the next workday", () => {
    const state = createDefaultState();
    ...
    state.assignments = generateSchedule(state, "2026-08-14").assignments;
    expect(
    ...
    state.assignments = generateSchedule(state, "2026-08-16").assignments;
    expect(
      state.assignments.find((item) => item.position === "H02")?.staffId
    ).toBe(replacement!.id);
    expect(
  ...
  it("allows an unavoidable priority-position repeat and reports the exact fallback reason", () => {
    const state = createDefaultState();
  ...
  it("reassigns an occupied qualified worker before breaking late-shift recovery protection", () => {
    const state = createDefaultState();
    const [protectedWorker, replacement] = state.staff
      .filter((person) => person.status === "正常")
      .slice(0, 2);
