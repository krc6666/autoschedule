...
import { applyFlightPlanReconciliation, addFlightsFromTemplates, deleteStaff, updateConfigurationField } from "./configuration-actions";
...
describe("configuration actions", () => {
  ...
  it("adds selected online-query templates without duplicating or overwriting existing flights", () => {
    ...
    state.flights = [{ ...structuredClone(existingTemplate!), id: "existing", bookedPassengers: 88 }];
    state.assignments = [{
      id: "assignment", flightId: "existing", flightNo: existingTemplate!.flightNo, positionRuleId: null,
      position: "临时岗位", staffId: null, staffName: "", startTime: existingTemplate!.startTime, endTime: existingTemplate!.endTime,
      workHours: 0, fatiguePoints: 0, remark: "", manualRemark: "", status: "manual"
    }];
    state.activeScheduleDate = "2026-07-27";

    const result = addFlightsFromTemplates(state, [existingTemplate!.id, newTemplate!.id, newTemplate!.id]);

    expect(result).toEqual({ added: 1, skipped: 2 });
    expect(state.flights.find((flight) => flight.id === "existing")?.bookedPassengers).toBe(88);
    expect(state.flights.filter((flight) => flight.flightNo === newTemplate!.flightNo)).toHaveLength(1);
    expect(state.assignments).toEqual([]);
    expect(state.activeScheduleDate).toBeNull();
  });
  ...
  it("atomically applies selected additions and confirmed removals", () => {
    ...
    state.flights = [
      { ...structuredClone(retainedTemplate!), id: "retained", bookedPassengers: 88, remark: "人工备注" },
      { ...structuredClone(removedTemplate!), id: "removed", bookedPassengers: 66 }
    ];
    state.assignments = [{
      id: "assignment", flightId: "retained", flightNo: retainedTemplate!.flightNo, positionRuleId: null,
      position: "临时岗位", staffId: null, staffName: "", startTime: retainedTemplate!.startTime, endTime: retainedTemplate!.endTime,
      workHours: 0, fatiguePoints: 0, remark: "", manualRemark: "", status: "manual"
    }];
    state.activeScheduleDate = "2026-07-27";
    const query: OnlineFlightQueryResult = {
      ...
      flights: [retainedTemplate!, addedTemplate!].map((template, index) => ({
        key: String(index), date: "2026-07-27", flightNo: template.flightNo, departureTime: "10:00",
        destination: "ICN", destinationCity: "Seoul", country: "韩国", countryCode: "KR"
      }))
    };
    const reconciliation = buildFlightPlanReconciliation(state, "2026-07-27", query);

    const result = applyFlightPlanReconciliation(state, reconciliation, [addedTemplate!.id], ["removed"]);

    expect(result).toEqual({ added: 1, removed: 1, skipped: 0 });
    expect(state.flights.map((flight) => flight.flightNo)).toEqual([retainedTemplate!.flightNo, addedTemplate!.flightNo]);
    expect(state.flights[0]).toMatchObject({ id: "retained", bookedPassengers: 88, remark: "人工备注" });
    expect(state.assignments).toEqual([]);
    expect(state.activeScheduleDate).toBeNull();
  });
  ...
  it("does not remove flights when reconciliation removal is blocked", () => {
    const state = createDefaultState();
    const query: OnlineFlightQueryResult = {
      date: "2026-07-28", nextDate: "2026-07-29", fetchedAt: "2026-07-26T05:49:13.434Z", sourceUrls: [], flights: []
    };
    const reconciliation = buildFlightPlanReconciliation(state, "2026-07-27", query);
    const flightIds = state.flights.map((flight) => flight.id);

    const result = applyFlightPlanReconciliation(state, reconciliation, [], flightIds);
