---
name: plan
description: 仅在 owner 明确要求计划时，在 autoschedule 项目内编写和执行单文件 plan.md；可用于中大型开发、跨模块修改、产品边界调整、架构硬化和生产级验收。
---

# Plan

仅在 owner 明确要求计划时使用本 skill。`plan.md` 是当前中大型任务的单文件执行合同，同时承担需求、事实、失败测试、目标、设计、任务、验证和收口记录。

## 触发后先做

1. 读取根目录 `AGENTS.md`。
2. 读取 `.agents/skills/autoschedule-dev/SKILL.md`。
