...
description: 维护 autoschedule 项目时使用。适用于修改实现、接口、数据、测试、事实文档、依赖、构建和运行配置，要求先确认事实，再完成实现、验证、文档同步与交付闭环。
---
...
5. owner 要求计划时，先读取 `.agents/skills/plan/SKILL.md` 并维护根目录 `plan.md`；owner 免除时不创建。
6. 任务涉及自动排班、规则优先级、值班、`KE166`、岗位完整性、疲劳、轮岗、人工调整或反馈时，完整读取 [排班维护经验](references/scheduling-maintenance.md)，只加载与规则维护有关的详细上下文。
...
- 行为保持型重构与规则行为修改分开进行；重构先建立语义等价基准，不在拆文件时顺手修正规则。
- 新增排班规则先确定规则 ID、执行阶段、冲突裁决和反馈证据，再进入对应职责模块；不得继续把新规则直接堆入 `src/domain/scheduler.ts`。
