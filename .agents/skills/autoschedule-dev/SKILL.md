...
3. 沿 `输入 -> 事实 -> 候选/硬约束 -> 求解 -> 锁定/补缺 -> 后置重排 -> 最终 assignments -> Store/UI/反馈` 建立影响矩阵。任何经过的阶段都必须有规则消费点或明确记录“不适用及原因”。
4. 对硬约束增加最终不变量，并增加后置流程反向测试：先构造合法结果，再尝试由恢复、轮岗、覆盖、压缩或人工入口改成非法结果，必须被拒绝或回退。
5. 对新增配置核对默认值、迁移、localStorage、Excel、归档、统计、页面编辑和往返测试；不适用项也要记录，不得默默跳过。
6. 交付前逐项核对需求条款、`spec.md`、旧行为、失败回退、真实入口和验证输出；测试全绿只能作为必要条件，不能作为唯一完成证据。

主 agent 不得把“局部测试通过”写成“功能完成”。若未执行真实浏览器入口、动态 import、Worker/WASM、Excel/localStorage 或其他受影响边界，必须在交付中明确标记未验证。
